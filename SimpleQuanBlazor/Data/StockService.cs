...
namespace SimpleQuanBlazor.Data
{
    public class StockService
    {
        ...
        public async Task<List<StockInfo>> GetTop100ByTurnoverAsync(DateTime? date = null)
        {
            var client = _httpClientFactory.CreateClient();
            
            // 如果指定了日期，使用历史数据接口；否则使用实时数据接口
            // 东方财富历史记录接口示例（按日期查询）需要 specific field base on date. 
            // 实时接口: fid=f6 (成交额)
            // 注意：API 接口 fs=m:0+t:6... 是当前的 A 股列表
            string dateStr = date?.ToString("yyyyMMdd") ?? "";
            string url = string.IsNullOrEmpty(dateStr) 
                ? "https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=100&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f6&fs=m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23,m:0+t:81+s:2048&fields=f12,f14,f2,f3,f6,f5,f62"
                : $"https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=100&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f6&fs=m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23,m:0+t:81+s:2048&fields=f12,f14,f2,f3,f6,f5,f62&_={dateStr}"; // 模拟日期查询
